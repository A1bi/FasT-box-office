...
- (void)payElectronically
{
    ...
    [[iZettleSDK shared] chargeAmount:total currency:nil enableTipping:NO reference:@"Abendkasse" presentFromViewController:self completion:^(iZettleSDKPaymentInfo *paymentInfo, NSError *error) {
        if (paymentInfo) {
            [self finish];
